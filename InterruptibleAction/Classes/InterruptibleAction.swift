...
public final class InterruptibleAction<Input, Output, Error: Swift.Error> {
    ...
    public init<State: PropertyProtocol>(state: State,
        ...
        interrupt = Action(enabledIf: Property(inner.isExecuting),
                           execute: { _ in SignalProducer(value: ()) })
    ...
    public convenience init<P: PropertyProtocol>(state: P,
                                                 execute: @escaping (P.Value, Input) -> SignalProducer<Output, Error>) {
    ...
    public convenience init<P: PropertyProtocol>(enabledIf isEnabled: P,
                                                 execute: @escaping (Input) -> SignalProducer<Output, Error>) where P.Value == Bool {
    ...
    public convenience init<P: PropertyProtocol, T>(unwrapping state: P,
                                                    execute: @escaping (T, Input) -> SignalProducer<Output, Error>) where P.Value == T? {
...
extension InterruptibleAction: BindingTargetProvider {
    /// Each new trigger will interrupt the executing of work.
...
extension InterruptibleAction where Input == Void {
    ...
    ///              be executed by the `Action`.
    public convenience init<P: PropertyProtocol, T>(unwrapping state: P,
                                                    execute: @escaping (T) -> SignalProducer<Output, Error>) where P.Value == T? {
    ...
    ///   - execute: A closure that produces a unit of work, as `SignalProducer`, to
    ///              be executed by the `Action`.
    public convenience init<P: PropertyProtocol, T>(state: P,
                                                    execute: @escaping (T) -> SignalProducer<Output, Error>) where P.Value == T {
